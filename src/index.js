...
function getAliases(state = {}) {
    //console.log(state);
    const {file: {opts: {filename} = {}} = {} } = state;
    const {config: configPath, findConfig: findConfig = false} = state.opts || {};
...
function convert(filePath, {aliasConf, extensionsConf}, {file: {opts: {filename}}, noOutputExtension}) {
  // const {file: {opts: {filename}}, opts: {config: configPath, findConfig: findConfig = false} = {}} = state;
    ...
    return {
        visitor: {
            ImportDeclaration(path, state) {
                const aliases = getAliases(path, state);
                path.node.source.value = convert(path.node.source.value, aliases, state) || path.node.source.value;
            },
            CallExpression(path, state) {
                ...
                const replacement = convert(filePath, aliases, state);
                if(replacement) {
                    path.node.arguments = [StringLiteral(replacement)];
                }
            },
        },
    };
